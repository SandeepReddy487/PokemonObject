...
FindPokemonObject()
PokemonObjectFromId(5)
PokemonObjectFromName("Bulbasaur")
PokemonObjectFromNum(100)
PokemonObjectFromWeaknesses("Rock")
PokemonObjectFromIdNameNum("1","Bulbasaur","001")
 PokemonObjectFromNextEvolution("002","Ivysaur")
function FindPokemonObject()
{
    console.log(PokemonObject);
}
function PokemonObjectFromId(PokemonId){
for(i=0;i<PokemonObject.length;i++){
    if(PokemonObject[i].id == PokemonId){
        console.log(PokemonObject[i])
    }
}
}
function PokemonObjectFromName(PokemonName){
    for(j=0; j<PokemonObject.length; j++){
    if(PokemonObject[j].name == PokemonName){
        console.log(PokemonObject[j])
}
    }
}
function PokemonObjectFromNum(PokemonNum){
    for(k=0;k<PokemonObject.length;k++){
    if(PokemonObject[k].num == PokemonNum){
    console.log(PokemonObject[k])
    }
}
}
function PokemonObjectFromWeaknesses(PokemonWeaknesses){
    for(var l=0; l<PokemonObject.length; l++)
    {
      for(var m=0; m<PokemonObject[l].weaknesses.length;m++)
      {
      if(PokemonObject[l].weaknesses[m] == PokemonWeaknesses)
      {
        console.log(PokemonObject[l].name);
      }
      }
      }
    }
function PokemonObjectFromIdNameNum(PokemonId,PokemonName,PokemonNum){
      for(n=0; n<PokemonObject.length; n++)
        {
          if(PokemonObject[n].id==PokemonId && PokemonObject[n].name==PokemonName && PokemonObject[n].num==PokemonNum)
          {
